...
let allProductsContainer = document.getElementById("all-products")
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button id="details-btn" onclick="productsDetails(${product.id})" class="btn btn-danger">Details</button></div>
      `;
...
let productsDetails = id => {
  ...
  detailDiv.innerHTML =
    `<div class="single-product" onclick="stopBubbling(event)">
    <div class="detailsClose" onclick="closeDetailsFunction()">X</div>
    <div>
    <img class="product-image" src=${productDetail[0].image}></img>
    </div>
    <h3>${productDetail[0].title}</h3>
    <p>Description: ${productDetail[0].category}</p>
    <p>Category: ${productDetail[0].description}</p>
    <h2>Price: $ ${productDetail[0].price}</h2>

    <p>Rating: <span>${productDetail[0].rating.rate}</span> on <span>${productDetail[0].rating.count} ratings</span>  </p>
    
    <button onclick="addToCart(${productDetail[0].id},${productDetail[0].price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
  </div>
`;
  detailsProductsDiv.appendChild(detailDiv)
  detailsProductsDiv.style.transform = 'translateX(0)'
  document.body.style.overflowY = 'hidden'
}

let closeDetailsFunction = () => {
  detailsProductsDiv.style.transform = 'translateX(-200%)'
